...
plot
b<-144*5
data3<-datacoba[577:676,c(1,2,4)]
plot(data3)
```

Data yang digunakan adalah data turbin pada tanggal 25 Maret 2023 dari pukul 00:00- 16.30. Peubah yang digunakan ada 2, yaitu Active Power sebagai Y, dan Wind Speed sebagai Xt.
...
Dari hasil tersebut, didapat bahwa seluruh peubah memiliki nilai $P-Value<0.05$. Hal ini menunjukkan bahwa seluruh peubah berpengaruh signifikan terhadap $y_t$. Adapun model keseluruhannya adalah sebagai berikut.

$$
\hat{Y_t}=-279.4939+103.61X_t+0.5571Y_{t-1}
$$
...
$$
\hat{Y_t}=-672.492+208.989X_t+ 1.188X_{t-1}+29.4525X_{t-2}
$$
...
finiteDLMauto(formula = ActivePower ~ WindSpeed,
              data = data.frame(train3), q.max=38, model.type = "dlm", error.type = "AIC", trace = FALSE)
```
...
model.dlm2 <- dlm(x = train3$WindSpeed,y = train3$ActivePower , q = 38)
summary(model.dlm2)
AIC(model.dlm2)
BIC(model.dlm2)
```
Dari hasil tersebut terdapat beberapa peubah yang berpengaruh signifikan terhadap taraf nyata 5% yaitu Intecept, $x_t$, $x_{t-9}$$x_{t-31}$, Adapun keseluruhan model yang terbentuk adalah

$$
\hat{Y_t}=-1097.5528+305.5846X_t+...-61.7322X_{t-38}
$$
...
$$
\hat{Y}=-265.74680+233.95381X_t-139.70672X_{t-1}+0.61015Y_{t-1}
$$
...
Hasil di atas menunjukkan bahwa hanya intercept, peubah $x_{t}$, $x_{t-1}$, $x_{t-2}$, $x_{t-4}$, $x_{t-13}$, $x_{t-15}$,$Y_{t-1}$ yang menunjukkan nilai-p pada peubah $\ge0.05$. Hal ini menunjukkan bahwa peubah $x_{t}$, $x_{t-1}$, $x_{t-2}$, $x_{t-4}$, $x_{t-13}$, $x_{t-15}$,$Y_{t-1}$ berpengaruh signifikan terhadap $y_t$. Model keseluruhannya adalah sebagai berikut:

$$
\hat{Y}=-404.4395+27.0540 X_t+....+0.2070Y_{t-2}
$$
